...
import ProductCard_mobile from "../src/components/ProductCard_mobile.vue";
    ...
    methods:{
        ...
        try{
            this.allProducts = await this.$axios.$get(`http://localhost:3000/api/books/${requestedOrder}`)     
        }catch(err){
            console.log(err)
        }
    },
     ...
     async sortProductsByName(requestedOrder) {
        try{
            this.allProducts = await this.$axios.$get(`http://localhost:3000/api/books&sortbyname/${requestedOrder}`)     
        }catch(err){
            console.log(err)
        }
    },
