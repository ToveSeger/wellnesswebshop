        ...
        <p v-else-if="$fetchState.error">Error while fetching products</p>   
         <ul v-else class="flex">
             <div class="sorting">
                ...
                <button  class="btn btn-light" @click="sortProductsByPrice('desc')"> 
                ...
                <button class="btn btn-light" @click="sortProductsByName('asc')">
                ...
                <button class="btn btn-light" @click="sortProductsByName('desc')">
...
import ProductCard_mobile from "../src/components/ProductCard_mobile.vue";
   ...
   // props:['product'],
    ...
    async fetch() {
        try{
            this.allProducts = await this.$axios.$get(`https://SG-wellnesswebshop-5650-mysql-master.servers.mongodirector.com/api/productbycategory/${this.categoryId}`)     
            }catch(err){
            console.log(err);
        }
    },
 ...
 methods:{
     ...
     async sortProductsByName(requestedOrder) {
        try{
            this.allProducts = await this.$axios.$get(`http://localhost:3000/api/insence&sortbyname/${requestedOrder}`)     
        }catch(err){
            console.log(err)
        }
    },
