...
import ProductCard from "../src/components/ProductCard.vue";
    ...
    async fetch() {
     ...
     methods:{
        ...
        try{
            this.allProducts = await this.$axios.$get(`http://localhost:3000/api/music/${requestedOrder}`)     
        }catch(err){
            console.log(err)
        }
    },
     ...
     async sortProductsByName(requestedOrder) {
        try{
            this.allProducts = await this.$axios.$get(`http://localhost:3000/api/music&sortbyname/${requestedOrder}`)     
        }catch(err){
            console.log(err)
        }
    },
